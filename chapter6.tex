...
\land (C_{A_n}(n) = n \rightarrow A_n=n))) \\
 ...
 前件を仮定して後件をしめす.\\
 $A_{n+1} \subseteq n+1$を仮定する.\\
 $A_n=A_{n+1}\cap n$とおくと,
 $A_n \subseteq A_{n+1}$であり,
 これと帰納法の仮定より\\
 $(\operatorname{Card}(A_n) = C_{A_n}(n) \leq n) \land (\BijectionMap{C_{A_n}\restriction_{{A_n}}}{A_n}{\operatorname{Card}(A_n)})\land (C_{A_n}(n) = n \rightarrow A_n=n)$は真.\\
 ($n \not\in A_{n+1}$) のとき\\
 (1) \\
 $A_{n+1}=A_n$.\\
 \begin{sub-block}{{\boldmath $\because$}\,}
 ...
 \end{sub-block}
 $n \not\in A$より,
 $C_{A_{n+1}}(n+1) = C_{A_{n+1}}(n)$.\\
 $A_{n+1} = A_n$より,
 $C_{A_{n+1}}(n) = C_{A_n}(n)$.
 また,
 $\operatorname{Card}(A_n) = \operatorname{Card}(A_{n+1})$.\\
 帰納法の仮定より,
 $\operatorname{Card}(A_n) = C_{A_n}(n)$なので\\
 $C_{A_{n+1}}(n) = C_{A_n}(n) = \operatorname{Card}(A_n) = \operatorname{Card}(A_{n+1})$.\\
 また,
 $C_{A_n}(n) \leq n$より,
 $C_{A_{n+1}}(n) \leq n+1$.\\
 (2) \\
